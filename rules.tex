...
\textbf{Aspects}: Aspects are highlighted like this \aspect{example aspect 3}. The tailing number indicates the value of the aspect. If an aspect has an additional number in square brackets (\aspect{example aspect 2[4]}), this denotes the \hyperref[sec:orge5f3575]{resistance} of the aspect. 
...
\section{Rules}
\label{sec:org01c6753}

\subsection{Dice Mechanics}
\label{sec:orge8a16dd}
\begin{short}
...
Sometimes it makes no sense to roll dice, but we want a constant value. Then we can \hyperref[sec:orge2a14ad]{take the average}. This means instead of rolling dice we just take the dice points as if it were the result of a roll. This is fair since it is also the average result of a roll.
...
The attributes value is the basis for the dice points one has available for \hyperref[sec:org3fb6a92]{checks}. 
\subsection{Aspects}
\label{sec:org0825370}
\begin{short}
...
Checks are rolled when the outcome of an action is not certain. Each check is bound to an \hyperref[sec:org100d9a2]{attribute}.

Add the points in the relevant attribute and all applicable \hyperref[sec:org0825370]{aspects} together. These are your dice points. You get the result of the roll in aspect points (AP) to create or change an Aspect.
...
\subsection{Traits}
\label{sec:orgf85aa3b}
\begin{short}
Traits allow characters to break rules in some way. Some checks that can only be performed because of a trait make those checks \hyperref[sec:org6442e93]{risky}. Traits can be bought for character points.
\end{short}
...
See the chapter \hyperref[sec:orgf204293]{List of Traits} for examples.
\subsection{Karma}
\label{sec:org7d1a075}
\begin{short}
...
\label{sec:org576650c}
\begin{short}
The RV of an item is the sum of all attached aspects AP. For multiple use items, remember that \hyperref[sec:org43aca7e]{enduring aspects} cost double the AP.
\begin{itemize}
...
\label{sec:org28cccee}
Resources can be gathered with a check and their RV as difficulty.
\section{Optional Rules}
\label{sec:org4cb49fe}
\subsection{Magic}
\label{sec:org32d7340}

Magic gives a huge narrative flexibility to explain aspects. To balance this out any checks made using magic should be \hyperref[sec:org6442e93]{risky checks}.
...
\label{sec:org11cd87e}

The GM may allow players retroactively having performed some action. For example having placed a trap beforehand. To balance this any check on such an action should be a \hyperref[sec:org6442e93]{risky check}.
\subsection{Quicker Battles}
\label{sec:org04444f7}
...
\label{sec:orgdf3400b}

Often you don't need highly specific attributes. Use \hyperref[sec:orgc7e8404]{less precise attributes} unless you really need more precision.
\begin{itemize}
...
\label{sec:orgbabc023}
Let the player make a check to create the \aspect{taunted by ..} aspect. This may be opposed by the other party. If the aspect is created then it hinders any attack on someone else. It may also impact other actions. It may be a smart choice to make the \aspect{taunted by ..} aspect \hyperref[sec:org43aca7e]{enduring} and \hyperref[sec:org687a017]{affecting the whole group}. 
\subsection{How To: Mind Control}
\label{sec:org2fa8634}
...
\begin{pwexample}
Marty the mage casts his mind control spell on an orc bandit with the words "Fight for me". Marty has 9 AP from his check, while the orc has only 3 to defend against this. This means that Marty can create the aspect with 6 AP. Since he wanted an \hyperref[sec:org43aca7e]{enduring aspect} he creates \aspect{Fight for Marty 3}, The orc has only 3 Will, so he starts attacking his compatriouts. 
\end{pwexample}
\subsection{How To: Healing}
\label{sec:org9aa43bc}
...
\label{sec:org026b136}
The following are examples of NPCs and monsters. They are all created using the rules for \hyperref[sec:org5f59ba8]{Character Creation}. 
...
\label{sec:org6329265}
\subsection{Character Builds}
\label{sec:org6c84961}
...
\end{multicols}
\normalsize
\section{One Shot: Sabotage during a space battle (Scy-Fi)}
\label{sec:org743f8fb}

\subsection{Introduction}
\label{sec:orgeecb7be}
...
\subsection{Cast}
\label{sec:org743c04c}

\subsubsection{Alex the Hacker}
\label{sec:orgdb0decd}
...
\end{npc}
\subsection{NPCs}
\label{sec:org4c0758b}

\subsection{Rules}
\label{sec:org4e37dcd}
...
\label{sec:org64a7426}

\href{images/Space Ship.png}{Space Ship Plan}
\subsubsection{Weapons Bays}
\label{sec:org86f0bda}
Ships Values
\begin{itemize}
\item Attack 120
\item Defense 0
...
\end{itemize}
\section{One Shot: In the Necromancers Dungeon (Medieval Fantasy)}
\label{sec:org1010b65}
...
A magical focus for the necromantic Arts
\item Dagger of Pain 6
\end{itemize}
\end{npc}

\begin{npc}{Guard}{at}{3 3 3 3 3 3 3 3}{0}
Aspects:
\begin{itemize}
\item 
\end{itemize}
...
\begin{npc}{}{at}{3 3 3 3 3 3 3 3}{0}
Aspects:
\begin{itemize}
\item 
\end{itemize}
...
\end{npc}
\subsection{Map}
\label{sec:org79dc50a}

\subsubsection{Dungeon (-1 floor)}
\label{sec:org1dff341}
CR +0
...
\label{sec:orgf672b2e}
CR +0
\subsubsection{Dining Room (1 floor)}
\label{sec:org1bb8016}
CR +0
...
\end{itemize}
. High up 6
\section{One Shot: A night at the Hospital (Modern Horror)}
\label{sec:org1d306b9}
...
\end{itemize}
\subsection{Cast}
\label{sec:org9b31f26}

\subsubsection{History Student}
\label{sec:orge770cb4}
...
\subsection{Map}
\label{sec:org12c0ceb}

\subsubsection{Pharmacy (1 Floor)}
\label{sec:org1df3adf}
...
\label{sec:org9ba3b40}

Stalked by the shadows. If one of the shadows catches you it will try to carry you off into the Orthopedic ward. There they will Break your limbs.
\subsubsection{Surgery theathres (2 floor)}
\label{sec:org00c7867}
