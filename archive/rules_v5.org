...
#+begin_comment
# Dice Sym https://anydice.com/program/31fae

function: pw N:n skill S:n {
 result: N <= S
}
...
loop S over {2..5} {
  loop N over {2..6} {
    output [lowest of 6 and Nd[pw d6 skill S]] named "[N]d|[S]"
  }
}
#+end_comment
...
*Risky checks:*
If a check is risky the character might incur something bad if the check fails. If the check fails the difficulty in effect point is used to incur effects to the characters disadvantage, the minimum is 1. 
...
For each point of advantage you can reroll one die per round (or roll one more die and remove the worst). For each point of disadvantage you have to reroll one success (or roll one more die and remove the best). A point of advantage and a point of disadvantage cancel each other out.
...
| *Dif* | Description | *CP* | *Good for ..* |
|-------+-------------+------+---------------|
|     0 | Very easy   |   15 | unskilled     |
|     1 | easy        |   25 | beginner      |
|     2 | normal      |   30 | experienced   |
|     3 | hard        |   35 | master        |
|     4 | very hard   |   40 | grand master  |
|     5 | extreme     |   45 | legend        |

#+ATTR_LATEX: :align c|c|l
| *Dif* | Description | *Attr + Skill* | *Good for ..* |
|-------+-------------+----------------+---------------|
|     0 | Very easy   |             4  | unskilled     |
|     1 | easy        |             6  | beginner      |
|     2 | normal      |             8  | experienced   |
|     3 | hard        |             10 | master        |
|     4 | very hard   |             11 | grand master  |
|     5 | extreme     |             12 | legend        |
...
#+begin_quote
*Plot Armor* (1): Each scene you can discard a point of damage you would take. The plot armor only apllies to one of the categories (Physical, Mental, Social) This trait can be taken multiple times.
#+end_quote
...
#+begin_quote
*Armored* (5): You have a natural armor physical armor of 1. Can be taken multiple times.
#+end_quote
...
#+begin_quote
*Shape-shift:* Take the form of another being. The new forms max CP depends on the EPs. Per 1 EP get 15 CP (max is the characters total CP). The effect holds until the end of the scene or until the end of the concentration. The dice spend on concentration are counted as scene damage after the effect ends. The new forms attributes and traits overwrite those of the character for the duration of the effect. Damge is taken per form separately. It takes 1 round to end the transformation.
#+end_quote

#+begin_quote
*Alter Form:* You can alter your body. Per 1 EP you can spend 5 CP on traits and attributes to alter your body. 
#+end_quote
...
#+begin_quote
*Average Citizen* (32 CP)
Ph:2, Me:2, So:2, Re:2
#+end_quote

#+begin_quote
*Goblin* (34 CP)
Ph:2, Me:1, So:1, Re:1, Bandit 3, Night-vision
#+end_quote
...
#+begin_quote
*Wolf* (38 CP)
Ph:3, Me:1, So:2, Re:0, Pack-hunter 4
#+end_quote

#+begin_quote
*Guard* (78 CP)
Ph:4, Me:4, So:4, Re:4, City-guard 3
#+end_quote
...
#+begin_quote
*Bandit* (73 CP)
Ph: 5, Me: 3, So: 3, Re: 2, Banditry 3
#+end_quote
...
#+begin_quote
*Orc Veteran* (83 CP)
Ph: 7, Me: 4, So: 3, Re: 3, Warlord 4, Nightvision, Maneuver Reckless Attack (4 damage, 2 difficulty)
#+end_quote

#+begin_quote
*Orc Warrior* (47 CP)
Ph: 3, Me: 2, So: 1, Re: 2, Bandit 3, Nightvision, Maneuver Reckless Attack (2 damage, 1 difficulty)
#+end_quote

#+begin_quote
*Giant Spider* (59 CP)
S: 2, D: 4, W:2, I:2, E:1, C:1, G:2, F:1, Ambush Predator 3,

Trait: Nightvision

Trait: Spider Webs
Can create spider webs. These can trap prey. The spider can feel vibrations in the net. The Spider can climb the net.

Maneuver:
Cast Net (2 disa
Lay Trapnet (1 d trigger, 2 difficulty)
#+end_quote
...
Maneuver: Mightl 2, Damage 2, Gain Advantage 2)
A heavy attack tdeep but also throws the target to the ground and stunns.
#+end_quote
...
** Exhausting Combat
To limit the duration of a combat scene apply this rule. If in one round no party takes any damage, then apply 1s damage to each party from the exhaustion of combat. 
...
- Yes or no question
- Roll 2D6
- The threasholds are 4|7|9
- If the answer is likely then increase all threasholds by 1
- If the ansert is unlikely then decrease all threasholds by 1
- Compare the rolled result with the threashold
  - If lower or equal to first threashold then the answer is exceptional yes
  - If lower or equal to second threashold then the answer is yes
  - If lower or equal to third threashold then the answer is no
  - If higher then third threashold then the answer is exceptional no
  - If exactly 7 then add a random event
