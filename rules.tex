...
\textbf{Aspects}: Aspects are highlighted like this \aspect{example aspect 3}. The tailing number indicates the value of the aspect. If an aspect has an additional number in square brackets (\aspect{example aspect 2[4]}), this denotes the \hyperref[sec:orgf0a3c53]{resistance} of the aspect. 
...
Sometimes it makes no sense to roll dice, but we want a constant value. Then we can \hyperref[sec:org14ee2bf]{take the average}. This means instead of rolling dice we just take the dice points as if it were the result of a roll. This is fair since it is also the average result of a roll.
...
The attributes value is the basis for the dice points one has available for \hyperref[sec:org1d27f17]{checks}. 
\subsection{Aspects}
\label{sec:orgfa4521d}
\begin{short}
...
\subsection{Traits}
\label{sec:org055e08d}
\begin{short}
Traits allow characters to break rules in some way. Some checks that can only be performed because of a trait make those checks \hyperref[sec:orgc05fe30]{risky}. Traits can be bought for character points.
\end{short}
...
See the chapter \hyperref[sec:orge17e5e7]{List of Traits} for examples.
\subsection{Karma}
\label{sec:org79f124e}
\begin{short}
...
Let the player make a check to create the \aspect{taunted by ..} aspect. This may be opposed by the other party. If the aspect is created then it hinders any attack on someone else. It may also impact other actions. It may be a smart choice to make the \aspect{taunted by ..} aspect \hyperref[sec:orgd4ba133]{enduring} and \hyperref[sec:orgdb6fd9e]{affecting the whole group}. 
...
\begin{pwexample}
Marty the mage casts his mind control spell on an orc bandit with the words "Fight for me". Marty has 9 AP from his check, while the orc has only 3 to defend against this. This means that Marty can create the aspect with 6 AP. Since he wanted an \hyperref[sec:orgd4ba133]{enduring aspect} he creates \aspect{Fight for Marty 3}, The orc has only 3 Will, so he starts attacking his compatriots. 
\end{pwexample}
...
\subsection{Cast}
\label{sec:org587942b}

\subsubsection{Alex the Hacker}
\label{sec:org3b611d8}
