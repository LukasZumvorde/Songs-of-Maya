...
\textbf{DP}: Means dice points (see \hyperref[sec:org016b36e]{Dice Mechanics})

\textbf{AP}: Means aspect points (see \hyperref[sec:org36f9986]{Checks})

\textbf{AV}: Means aspect value (see \hyperref[sec:orgd63f1b0]{Aspects})
\section{Rules}
\label{sec:orgc94417f}

\subsection{Dice Mechanics}
\label{sec:org016b36e}
\begin{short}
Whenever a dice roll is required you get dice points (DP) (see \hyperref[sec:org36f9986]{Checks}). The result of a roll are aspect points (AP). There are many ways to perform a roll. This is only the default way to do it. Read the entire section \hyperref[sec:org016b36e]{Dice Mechanics} to develop your own.
...
\item Rolling any dice twice and subtracting the second roll from the first is worth exactly \(0 DP\)
\item A dice that always rolls the constant value \(N\) is worth \(N\) DP
\end{itemize}
...
Sometimes it makes no sense to roll dice, but we want a constant value. Then we can \hyperref[sec:orgdcb8fdf]{take the average}. This means instead of rolling dice we just take the dice points as if it were the result of a roll. This is fair since it is also the average result of a roll.
...
The attributes value is the basis for the dice points one has available for \hyperref[sec:org36f9986]{checks}. 
\end{short}
\subsection{Aspects}
\label{sec:orgd63f1b0}
\begin{short}
...
\item Remove: To remove an aspect you need to make a check. Reduce the AV of the aspect by the AP of the check. You may need to match the duration of the aspect or the GM can make apply again later.
\item Resistance: Aspects can resist being removed. Reduce any attempt to remove the aspect by its resistance. Each point of resistance costs 1 AP. The costs double in line with other modifications.
\end{itemize}
\end{short}

Aspects are a combination of a descriptors and a value. When invoked the AV (aspects value) is added or subtracted from the DP (dice points) for \hyperref[sec:org36f9986]{checks}. An aspect is always bound to something. Be it a player, a NPC, an object, an event, or a zone. Be creative with aspects. Try to bind their effects to the value and they should stay balanced enough.
\paragraph*{Creating an Aspect}
\label{sec:orgffdf987}
...
\subsection{Checks}
\label{sec:org36f9986}
\begin{short}
Add points of an attribute any applicable aspects value (AV) to get the dice points (DP). For these you can buy one or more dice to roll (see \hyperref[sec:org016b36e]{dice mechanics}). The result of the roll are the aspect points (AP) of the check.
\begin{itemize}
\item Difficulty: Some aspects hinder a check. For them consider the AV to be negative instead.
...
Checks are rolled when the outcome of an action is not certain. Each check is bound to an \hyperref[sec:orgca8aecc]{attribute}.

Add the points in the relevant attribute and all applicable \hyperref[sec:orgd63f1b0]{aspects} together. These are your dice points. Use them to buy the dice to roll (see \hyperref[sec:org016b36e]{Dice Mechanics}). You get the result of the roll in aspect points (AP) to create or change an Aspect.
...
The default aspect created in combat is \hyperref[sec:orgf46c3dc]{\aspect{damage}}.
...
\subsection{Traits}
\label{sec:org21f6098}
\begin{short}
Traits allow characters to break rules of the game or the world in some way. Some checks that can only be performed because of a trait make those checks \hyperref[sec:org0eca69b]{risky}. Traits can be bought for character points.
\end{short}
...
See the chapter \hyperref[sec:org227d4db]{List of Traits} for examples.
\subsection{Karma}
\label{sec:org113d582}
\begin{short}
...
\label{sec:org8b065d4}
\begin{short}
Items behave exactly like aspects if they have an AV. For multiple use items apply the rules for \hyperref[sec:org1d03a82]{enduring aspects}. 
\begin{itemize}
\item Equipment: Characters may initially have any item that they could acquire using buying, crafting, or gathering if they always take the average on checks.
\item Buying: Make a finances check to create the items aspects. On success get the item and a negative aspect \aspect{Expenses} worth the same amount of AP on your finances for some time.
...
\item Condition: Apply the rules for \hyperref[sec:org1d03a82]{enduring aspects} if the item is in use. If its time has run out you can still use it, but it may break at any time, especially on a failed check.
\end{itemize}
\end{short}

Items are in essence just named things with one or more attached aspects. For most items their aspects are obvious and don't need to be mentioned specifically. For example if you use a sword with AV 4, then you can just assume that it has an aspect aiding in a fight for 4 points. Some items may be \hyperref[sec:org1d03a82]{enduring}, they can be used multiple times. Other items are a one use item like a first aid kit. 
\subsubsection{Equipment}
\label{sec:orgf31b32e}
...
\label{sec:org7b0f6e8}

The GM may allow players retroactively having performed some action. For example having placed a trap beforehand. To balance this any check on such an action should be a \hyperref[sec:org0eca69b]{risky check}.
\subsection{Quicker Battles}
\label{sec:org67e5ea6}
...
\subsection{Adjust Power-level}
\label{sec:org04a4d3d}
...
\newpage
\section{Advice}
\label{sec:orgb341293}
\subsection{Gameplay Notes}
\label{sec:orgbd753e5}
...
\label{sec:org03995eb}

Often you don't need highly specific attributes. Use \hyperref[sec:org6881f2e]{less precise attributes} unless you really need more precision.
\begin{itemize}
...
\item For group challenges, the sum of all players CR is a good value for the aspect.
...
\label{sec:orge2c9cea}

Let the player make a check to create the \aspect{taunted by ..} aspect. This may be opposed by the other party. If the aspect is created then it hinders any attack on someone else. It may also impact other actions. It may be a smart choice to make the \aspect{taunted by ..} aspect \hyperref[sec:org1d03a82]{enduring} and \hyperref[sec:org2e3c993]{affecting the whole group}. 
\subsection{How To: Mind Control}
\label{sec:org7744361}
...
\begin{pwexample}
Marty the mage casts his mind control spell on an orc bandit with the words "Fight for me". Marty has 9 AP from his check, while the orc has only 3 to defend against this. This means that Marty can create the aspect with 6 AP. Since he wanted an \hyperref[sec:org1d03a82]{enduring aspect} he creates \aspect{Fight for Marty 3s}, The orc has only 3 Will, so he starts attacking his compatriots. 
\end{pwexample}
\subsection{How To: Healing}
\label{sec:orge099f8c}
...
\label{sec:orgc2a871d}
To Bribe someone you need to give them more than they can normally comfortably afford. This means you need to create the \aspect{bribe} with an AV of at least their finances value plus applicable aspects. The \aspect{bribe} may need to be \hyperref[sec:org1d03a82]{enduring} to cover a longer time span.
...
\begin{quote}
\textbf{Hide in Plain Sight} (25): You can disappear from sight, like a ninja with a smoke bomb.
\end{quote}

\begin{quote}
\textbf{Short Range Teleportation} (90): You can disapear from your current position and instantaneously appear somewhere else. The target has to be in a short range to you, maybe a couple of zones at most. The ability is limited by some condition like: "you have to see the place", "You have to have been there", "You can only teleport from one shadow to another".
\end{quote}

\begin{quote}
\textbf{Long Range Teleportation} (180): You can disapear from your current position and instantaneously appear somewhere else. There is practically no limit to the range. The ability is limited by some condition like: "you have to see the place", "You have to have been there", "You can only teleport from one shadow to another".
\end{quote}
\subsection{List of NPCs}
\label{sec:orgf1011c3}
The following are examples of NPCs and monsters. They are all created using the rules for \hyperref[sec:org37e1853]{Character Creation}. 
...
\begin{itemize}
\item they symbolize what people think about the character
...
\label{sec:org4430987}

A Sci-Fi scenario.
\subsection{Introduction}
\label{sec:org992ebe6}
...
\end{npc}
\subsection{NPCs}
\label{sec:orgb337945}

\subsection{Rules}
\label{sec:org85b931c}
...
\item \aspect{Plasma Pistol 2}
\end{itemize}
\item \aspect{High Power Consumption 5}
\item \aspect{Many nooks and crannies 4}
\end{itemize}
\subsubsection{Auxiliary Fusion Reactors}
\label{sec:org9a82b04}
Ships Values
\begin{itemize}
\item Attack 0
\item Defense 0
...
\item \aspect{Plasma Rifle 3}
\end{itemize}
\item \aspect{High Power Consumption 8}
\end{itemize}
\subsubsection{Brig}
\label{sec:orgc8e3446}
Ships Values
\begin{itemize}
\item Hull 50
...
\end{npc}
\subsection{Map}
\label{sec:orgae27e7d}

\subsubsection{Dungeon (-1 floor)}
\label{sec:org77ac1d3}
CR +0
...
\label{sec:orgea5b3f6}
CR +0
\subsubsection{Dining Room (1 floor)}
\label{sec:org1180c7b}
CR +0
...
\begin{npc}{Psychology Student}{at}{2 3 3 3 4 4 2 3}{36}
Aspects:
\begin{itemize}
\item \aspect{Psychology 3}
...
\subsection{Map}
\label{sec:org5c6de93}

\subsubsection{Pharmacy (1 Floor)}
\label{sec:orgbf63578}
...
\item You can find caffeine to stay awake.
\end{itemize}
\subsubsection{Kitchen (1 floor)}
\label{sec:org1f70d7e}
...
\begin{itemize}
\item \aspect{A feeling of strong unease and danger 2}
\end{itemize}
...
\label{sec:org8454b21}

Stalked by the shadows. If one of the shadows catches you it will try to carry you off into the Orthopedic ward. There they will Break your limbs.
\subsubsection{Surgery theaters (2 floor)}
\label{sec:org8382291}
...
\end{center}
\newpage
\section{Simplified Rules}
\label{sec:org07c5be5}

\subsection{Aspects}
\label{sec:org606bb0a}
Aspects don't get values.
\subsection{Attributes}
\label{sec:org7b165da}
...
\begin{itemize}
\item If a character uses a very low attribute in a check, it acts like two negative aspects.
\item If a character uses a low attribute in a check, it acts like a negative aspect.
\item If a character uses a high attribute in a check, it acts like a positive aspect.
\item If a character uses a very high attribute in a check, it acts like two positive aspects.
\end{itemize}
\subsection{Checks}
\label{sec:orgbe0e7c1}
...
\subsection{Contests}
\label{sec:org22e34eb}
Each round two or more parties each roll a check. The one with the highest difference of the dice wins. If the differences are the same then the one with the higher result on the first die wins. If it is still even, then it becomes a stalemate.
...
\subsection{Traits}
\label{sec:org02bf430}
As normal.
\subsection{Character Creation}
\label{sec:orgd7c88d4}
You have 240 CP. You can buy the following from them.
\begin{itemize}
\item 1 Attribute from normal to high (30CP)
\item 1 Attribute from normal to low (-30CP)
\item 1 Aspect (15CP)
\item 1 Trait (25CP)
\end{itemize}
\end{document}
