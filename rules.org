...
*Dice shorthand*: $3 d 6$ means rolling three six sided dice and summing up the results. $1 d_0 2$ means rolling 1 dice whose sides go from 0 to 2 (three sides in total).
...
We use a pool of $d_0 2$ to roll, although optional rules for [[#sec:alternativedicemechanics][alternative dice mechanics]] exist and are strongly recommended once you get the hang of things. Since it is hard to find dice with the sides 0, 1, and 2 I recommend 2 alternatives.
...
Sometimes it makes no sense to roll dice, but we want a constant value. Then we can [[#sec:checks_average][take the average]]. This means instead of rolling dice we just take the dice pool as if it were the result of a roll. This is fair since it is also the average result of a roll.
...
:END:
In some situations it may be good idea no to roll any dice at all. In those cases you can just assume that the average result was rolled. This means that you get the =dice pool= in =AP= out of the check.
...
- If in doubt then determine the order by comparing the dice pools of the parties. The highest one begins.
...
** How to Rule: Extremely Small Creatures
...
Mind control is similar to taunting. Create an aspect like \aspect{Mind Controlled by ..}. Any actions that would conflict with this aspect are hindered by it (for example attacking the controller). If the mind controll is stronger than the victims then it will generally follow orders.
