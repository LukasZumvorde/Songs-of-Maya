...
*DP*: Means dice points (see [[#sec:dice][Dice Mechanics]])

*AP*: Means aspect points (see [[#sec:checks][Checks]])

*AV*: Means aspect value (see [[#sec:aspects][Aspects]])
...
#+begin_short
Whenever a dice roll is required you get dice points (DP) (see [[#sec:checks][Checks]]). The result of a roll are aspect points (AP). There are many ways to perform a roll. This is only the default way to do it. Read the entire section [[#sec:dice][Dice Mechanics]] to develop your own.

When a roll is required, roll one six sided die and add the result to your DP. Then roll another six sided die and subtract it from your DP. The result are your AP.

$AP = DP + 1d6 - 1d6$
#+end_short

The dice points (DP) are calculated from the attributes and aspects. They can be used to buy dice and constant points for the roll of a check. When dice are rolled, the results are added together, together with any constant points to get the result of the check. They are called the aspect points (AP). AP can be used to create aspects (see [[#sec:aspects][Aspects]]). DP can only be used in a single roll, and not be kept for future rolls.
...
- A die is worth exactly its average result in DP.
  ...
  - Rolling any dice twice and subtracting the second roll from the first is worth exactly $0 DP$
  - A dice that always rolls the constant value $N$ is worth $N$ DP
...
To keep all possible results evenly distributed you should favor to roll just a single die, or at most two dice. Pick higher factors ($F \times d10$) if you have significantly more DP available. Don't be afraid of the fractional DP costs. 
...
#   - $1 d_0 N$ is worth $\frac{N}{2}$ DP
#   - Throwing a N sided die twice and subtracting the lower result from the higher is worth $\frac{N}{3}$ DP
#   - A dice that always rolls a constant value $N$ is worth $N$ DP
#   - Rolling any dice twice and subtracting the second roll from the first is worth exactly $0 DP$
# - It is possible for costs to contain a fraction of a DP.
# - Any dice multiplied by a factor $F$ is worth the price of a single die multiplied by this factor $F$
...
# If you find the dice mechanics confusing, then you can always use the following. Roll $1d6$ add it to your dice points (DP), roll $1d6$ again and subtract it from your $DP$. The result are your aspect points (AP). 
...
#+begin_short
There are the 8 attributes Strength, Dexterity, Will, Intellect, Empathy, Charisma, Gear, and Finances. Each attribute gets a whole number value. The higher the value the greater things a character can achieve.
...
Two attributes each can be combined to a attribute category. Having a point in a category means having a point in all its attributes and vice versa. All categories can be combined to a single value called competence.
...
- Create: Make a check. For each aspect point (AP) from the checks result you can increase the AV of the new aspect by one.
- Use: Add the AV of the aspect to the DP for the check. Any AV point can only be used once per round. 
- Multiple Targets: To create an aspect that effects an area or multiple targets, double the AP cost per AV.
- Duration: By default an aspect can be used once. To make it apply every round until the duration is over, double the cost per AV per duration category (once, scene, mission, campaign). 
- Acting: Some aspects can perform actions on their own. For this use the AV as the DP for the check.
- Remove: To remove an aspect you need to make a check. Reduce the AV of the aspect by the AP of the check. You may need to match the duration of the aspect or the GM can make apply again later.
- Resistance: Aspects can resist being removed. Reduce any attempt to remove the aspect by its resistance. Each point of resistance costs 1 AP. The costs double in line with other modifications.
#+end_short

Aspects are a combination of a descriptors and a value. When invoked the AV (aspects value) is added or subtracted from the DP (dice points) for [[#sec:checks][checks]]. An aspect is always bound to something. Be it a player, a NPC, an object, an event, or a zone. Be creative with aspects. Try to bind their effects to the value and they should stay balanced enough.
...
The enduring aspect \aspect{Damage} is also the name of an abstraction for other more precise aspects like \aspect{broken leg} or \aspect{intimidated}. Its only purpose is to speed up combat when detail is not wanted. As such it acts a bit differently. Damage does not hinder one during a contest. After a contest it is converted to another aspect of equal value. Often times this enduring aspect is more enduring. If one has more \aspect{damage} which would hinder one in an action, than DP available for this action, then one can not perform this action. This is generally the point at which one is considered incapacitated.
...
#+begin_short
Add points of an attribute any applicable aspects value (AV) to get the dice points (DP). For these you can buy one or more dice to roll (see [[#sec:dice][dice mechanics]]). The result of the roll are the aspect points (AP) of the check.
- Difficulty: Some aspects hinder a check. For them consider the AV to be negative instead. 
- Collaboration: Add either the DP for the checks or the AP of all checks made together.
...
:END:
In some situations it may be good idea not to roll any dice at all. In those cases you can just assume that the average result was rolled. This means that you get the =DP= in =AP= out of the check. 
...
- A Trait typically costs 25 CP but can vary widely. Negative traits can even have a negative price.

A typical player character will have:
- all attributes with value 3 on average
- 3 aspects with value 4
- 1 trait
#+end_short
...
#+begin_short
Items behave exactly like aspects if they have an AV. For multiple use items apply the rules for [[#sec:aspects_enduring][enduring aspects]]. 
- Equipment: Characters may initially have any item that they could acquire using buying, crafting, or gathering if they always take the average on checks.
- Buying: Make a finances check to create the items aspects. On success get the item and a negative aspect \aspect{Expenses} worth the same amount of AP on your finances for some time.
- Crafting: Make a check to create the items aspects. Resources used for the check get used up.
- Gathering: Make a check to create the items aspects. 
- Condition: Apply the rules for [[#sec:aspects_enduring][enduring aspects]] if the item is in use. If its time has run out you can still use it, but it may break at any time, especially on a failed check.
#+end_short

Items are in essence just named things with one or more attached aspects. For most items their aspects are obvious and don't need to be mentioned specifically. For example if you use a sword with AV 4, then you can just assume that it has an aspect aiding in a fight for 4 points. Some items may be [[#sec:aspects_enduring][enduring]], they can be used multiple times. Other items are a one use item like a first aid kit. 

*** Equipment
Characters may initially have any item on them that they could acquire by buying, crafting, or gathering if they always take the average on checks. They must be able to carry all that gear on them or if it is part of their household it must fit in their normally furnished home. Apply reason as necessary.
...
*** Condition
Items may break, even when used normally. Use the rules for [[#sec:aspects_enduring][enduring aspects]] to create the item and determine its durability. The items don't immediately break if the aspects time has run out, but the GM may decide that they do at any time afterwards. This could for example be done if a check with them fails. To repair an item make a crafting check, if it makes sense for the item. Depending on the AV the item may not be repaired fully.
...
To limit the amount of spells being cast, you may rule that a spell has to be kept active. This means that each active spell acts as a negative aspect for each additionally cast spell. 
...
The challenge rating (CR) is a number serving as a quick reference for how hard aspects to overcome should be or how strong enemies should be, When creating aspects that the players must overcome use the CR as the AV (aspect value). Then creating enemies that the players must fight set their Competence or attribute to the CR.
...
- For group challenges, the sum of all players CR is a good value for the aspect.
...
** How To: Fall Damage
Assuming your characters are humans then falling creates damage. You get a damaging aspect worth 4 AP per 1m of height of the fall. This can be reduced with an appropriate check. Remember that enduring aspects (including damage from falling) cost double the AP.

#+begin_pwexample
A burglar jumps from rooftop to rooftop to escape the guards. One of the chasms is to wide though and he plummets onto the street 4m (16 AP) below. He makes a check with his dexterity to mitigate this fall. On this check he gets 10 AP. Thus he gets only 6 AP worth of damage. The GM decides that he has a \aspect{sprained ankle 3} for the rest of the scene.
#+end_pwexample

#+begin_pwexample
The spy is hanging on a zip-line rappelling from one sky scraper to the next. The enemy guards cut the rope. Now the spy is falling to the balcony far below his original target. The fall would be roughly 10m (40 AP). Even with his skills this is deadly. He rolls only 3 AP on his mitigation check. Thus he crashes to the ground receiving /aspect{Broken bones all over the body 4c} and /aspect{disoriented 5r} form the fall. Luckily his constitution is quite good \aspect{Physical 6} so he survives, but barely.
#+end_pwexample

** How To: Bribe
To Bribe someone you need to give them more than they can normally comfortably afford. This means you need to create the \aspect{bribe} with an AV of at least their finances value plus applicable aspects. The \aspect{bribe} may need to be [[#sec:aspects_enduring][enduring]] to cover a longer time span.

#+begin_pwexample
The group got caught in the streets by a guard after the curfew. They try to bribe him to convince him to act as if he hadn't seen them. For this they need 5 AP to match the guards finances value of 5.
#+end_pwexample

#+begin_pwexample
The crime boss intends to bribe the police commissioner to keep the police force away from his business until he gets the next big deal done. The commissioner has \aspect{finances 6} and also a \aspect{sense of duty 4}. So bribing him for this limited time (until the end of the mission) would cost 40 AP.
#+end_pwexample
...
** How To: Sacrifices
A mage sacrificing a maiden in order to summon a daemon or a priest sacrificing a goat to please his god are common themes in stories. In game you will want to use such a sacrifice as a helping aspect. Also sacrifices of objects are feasible. For objects set its total $\frac{AP cost}{2}$ as the sacrifices value. For animals and similar set its $CR \times 2$ as its sacrifices value. For people set their $CR \times 4$ as the sacrifices value.
...
#+begin_quote
*Friend of Nature* (25): You can talk to the forces of nature and have a chance to convince them to help you. This can be asking, a bird what he has seen, letting yourself be concealed by a bush or calling a wild bear to aid you in combat.
#+end_quote

#+begin_quote
*Illusionist* (25): You are adapt at creating illusions. The bigger and more complex they get the harder this is.
#+end_quote

#+begin_quote
*Speedster* (50): You have incredible speed. Others see only a blur when you sprint past them. This often gives you an advantage on dexterity checks, so you can re-roll them once per round. You can move double the distance per round.
#+end_quote

#+begin_quote
*Medium* (25): You can commune with ghosts and spirits. You have no control over them, but you can gain their attention.
#+end_quote

#+begin_quote
*Night-vision* (25): You can see in darkness as if it were light.
#+end_quote

#+begin_quote
*Sleepless* (25): You don't need sleep. This means you have a lot more time in a day, but you still need to rest from to much physical or mental exertion.
#+end_quote

#+begin_quote
*Flight* (60): You can fly. Be it with wings or otherwise. Your speed in flight is no different from your speed on land.
#+end_quote

#+begin_quote
*Tinkerer* (25): You can build wondrous mechanical marvels. From clocks up to steam powered automatons. 
#+end_quote

#+begin_quote
*Hacker* (25): You are not only proficient in computer science but you can even achieve movie worthy feats like stopping another car with only your laptop during a car chase. Tools not included.
#+end_quote

#+begin_quote
*Plot Armor* (25): Each scene you can disregard one new aspect or one change in an aspect representing damage. 
#+end_quote

#+begin_quote
*Short Weapon Fighting* (15): You can not get disadvantage because your weapons are to short compared to your opponent.
#+end_quote

#+begin_quote
*Unarmed vs. Armed* (15): You can fight against armed opponents even when you have no weapon without disadvantage.
#+end_quote

#+begin_quote
*Alchemist* (25): You can brew potions, salves and other things which create wondrous effects.
#+end_quote

#+begin_quote
*Shape Shifter* (25): You can alter the physical form of either yourself or that of others.
#+end_quote

#+begin_quote
*Seeer* (25): You have to ability to see glimpses of future, past and present. Both at your current position and over great distances. 
#+end_quote

#+begin_quote
*Amphibious* (25): You can live both underwater and on land.
#+end_quote

#+begin_quote
*Telepathic link to undead servants* (25): You can telepathically give orders or even see through the eyes of the undead creatures you raised. 
#+end_quote

#+begin_quote
*Hide in Plain Sight* (25): You can disappear from sight, like a ninja with a smoke bomb.
#+end_quote

#+begin_quote
*Short Range Teleportation* (90): You can disapear from your current position and instantaneously appear somewhere else. The target has to be in a short range to you, maybe a couple of zones at most. The ability is limited by some condition like: "you have to see the place", "You have to have been there", "You can only teleport from one shadow to another".
#+end_quote

#+begin_quote
*Long Range Teleportation* (180): You can disapear from your current position and instantaneously appear somewhere else. There is practically no limit to the range. The ability is limited by some condition like: "you have to see the place", "You have to have been there", "You can only teleport from one shadow to another".
#+end_quote
...
#+ATTR_LATEX: :options {Average Citizen}{co}{5}{0}
#+begin_npc
Aspects:
- None
Traits:
...
#+ATTR_LATEX: :options {Goblin}{ca}{3 2 2 2}{25}
#+begin_npc
Aspects:
- None
Traits:
...
#+ATTR_LATEX: :options {Ratling}{ca}{2 2 2 2}{6}
#+begin_npc
Aspects:
...
#+ATTR_LATEX: :options {Guard}{ca}{6 5 5 4}{6}
#+begin_npc
Aspects:
- City Guard 1
Traits:
...
#+ATTR_LATEX: :options {Dark Mage}{ca}{4 10 5 7}{43}
#+begin_npc
Aspects:
- Necromancer 3
\columnbreak
...
#+ATTR_LATEX: :options {Ogre}{at}{12 5 5 2 2 2 1 1}{}
#+begin_npc
Aspects:
- None
Traits:
...
#+ATTR_LATEX: :options {Zombie}{ca}{5 1 1 1}{25}
#+begin_npc
Aspects:
- None
Traits:
...
#+ATTR_LATEX: :options {Bandit}{ca}{6 5 5 4}{}
#+begin_npc
Aspects:
- None
Traits:
...
#+ATTR_LATEX: :options {Combat Drone}{ca}{3 1 1 1}{43}
#+begin_npc
Aspects:
- Shooting 3
Traits:
...
#+ATTR_LATEX: :options {Orc Veteran}{ca}{6 4 3 3}{37}
#+begin_npc
Aspects:
- Reckless and Bold 2
Traits:
...
#+ATTR_LATEX: :options {Orc Warrior}{ca}{5 4 2 2}{31}
#+begin_npc
Aspects:
- Reckless and Bold 1
Traits:
...
#+ATTR_LATEX: :options {Giant Spider}{at}{2 4 2 2 1 1 2 1}{37}
#+begin_npc
Aspects:
- Spider Webs 2
Traits:
...
* COMMENT Game-play Examples
:PROPERTIES:
:CUSTOM_ID: sec:examples
:END:

** COMMENT Character Builds
...
** COMMENT Example: Ambushed by Goblins

*GM* is the Game Master Matthew controlling the 4 goblin bandits (P: 2, M; 1, S: 1, Life of Banditry 1)
...
*GM:* So no \aspect{shallow ruts} and no \aspect{thick underbrush}?
...
- Write story points in a list when they come up and you feel like it
  - Whenever you create a new story point you may create a connection to an existing one.
...
Gods are real and numerous. When they have children with other gods or mortals their offspring sometimes inherit the spark of divinity. The gods are shaped and empowered by the believes of the mortals about them.

The player characters are born with this spark of divinity. Initially they are mostly like the mortals. But as interact more and more in the world, stories about their deeds start shaping their being and empower them.
...
- they symbolize what people think about the character
...
* Scenario: A galaxy of wonders

** Society
- Many alien species
- The galaxy is old and has many species in it. May an empire has rissen and fallen. Some seeded other life, some destroyed it.
- Common prefered conditions for life

  
** Technology
- Technological Improvements have stalled
- Common translator technology
- Common and shared technologies
- Almost no one planet/species is able to produce the best of all technologies themselves
- Interstellar Travel ist possible and common.
- Droids and Robots are perfeclty possible and where the technology allows for it common.
- Meele weapons are common. Many spaceships are very fragile for internal projectules and damage can be catastophic for the shoter as well as his target. Also some schields and armors exists that can block ranged weapons (for the most part). There exists many melee weapon types which can defeat armor, for example a plasma blade may be able to slowly melt itself through many a armor plate.
- Some particularly turbulent areas of hyperspace can be traversed with the help of Gates. These form a more stable corridor to fly though.
- Psynics exist. They are a advanced technology that is sometimes genectically implanted in (some indivuduals of) species and very rarely enacted by machines.
- The power for psyonics must come from something, however this something does not have to be physical but can also be metaphorical or otherwise immaterial.

** Ecology
- Hyperspace can not be savely entered near to bodies of mass
- In Hyperspace weird and chaotic currents exists, which makes travel sometimes fast and sometimes slow.
- Some prefer to stay awake during hyperspace travel, others prefer to be in stasis
- When not shieldet hyperspace tends to slowly change what enters it. (Metal becomes like glass, peoples personality changes, ..) there is no single pattern to the changes.
- AGI is possible but needs special materials as a substrate. These are rare. One particular AGI runs on Biological Brains of one particular sentient Species. The AI is otherwise benevolent but has to resort to this barbarity for its continued existence.

** History
- The galaxy has collided with another galaxy. An alliance of many powerfull empires wanted to prevent this. They build a massive machine around the black hole at the center of the galaxy to shift galaxy through hyperspace in order to prevent the collision. They failed spectacularly. Now the galaxys are mid collision and for centuries the hyperspace has become so turbulent that most plannets were isolated.
...
- 3 Engineers (CR 4)
  - \aspect{Plasma Pistol 2}
- \aspect{High Power Consumption 5}
- \aspect{Many nooks and crannies 4}
...
- 4 Guards (CR 5)
  - \aspect{Plasma Rifle 3}
- \aspect{High Power Consumption 8}
...
#+ATTR_LATEX: :options {Psychology Student}{at}{2 3 3 3 4 4 2 3}{36}
#+begin_npc
Aspects:
- \aspect{Psychology 3}
- \aspect{Barista 1}
...
- You can find caffeine to stay awake.
...
- \aspect{A feeling of strong unease and danger 2}
...
** Aspects
Aspects don't get values.

Aspects have no duration or act on multiple targets. Do treat them as enduring if it makes sense and however long it makes sense. 

** Attributes
There are the 4 Attributes: Physical, Mental, Social, Ressources.

Each attribute can only have one of the 5 states: Very Low, Low, Normal, High, Very High.

- If a character uses a very low attribute in a check, it acts like two negative aspects.
- If a character uses a low attribute in a check, it acts like a negative aspect.
- If a character uses a high attribute in a check, it acts like a positive aspect.
- If a character uses a very high attribute in a check, it acts like two positive aspects.

** Checks
Roll 2d6 one after the other. If the first is higher than the second then the check is a success. If it is lower then it is a failure. For each positive aspect you can change any of the two dice by one. For each negative aspect you have to do it to your disadvantage.

Roll a 1d10. Consider the 5 or higher a success if the die goes from 0 to 9 or consider the 6 or higher a success if the die goes from 1 to 10. For each positive aspect change the rolled number by 1 to your advantage. For each negative aspect change the number by 1 for your disadvantage.


** Contests
Each round two or more parties each roll a check. The one with better result wins. If it is even, then it becomes a stalemate.

The winner of the contest may create an aspect or remove an aspect. If a side has little or no chance to succeed a round of the contest, then it ends.

** Traits
As normal.

** Character Creation
You have 240 CP. You can buy the following from them.
- 1 Attribute from normal to high (30CP)
- 1 Attribute from normal to low (-30CP)
- 1 Aspect (15CP)
- 1 Trait (25CP)
